...
## 附录 A：后台核查清单（2026-09-04 补，用于回答「境外主体能不能开」）

登录 https://work.weixin.qq.com/wework_admin/frame ，电脑浏览器，管理员扫码。

**第 0 步 —— 先看主体：**「我的企业」→「企业信息」。看认证状态和主体所在地，确认注册时用的是什么主体。页面最底部的**企业ID 就是 `corpid`**，先抄下来。

**第 1 步 —— 决定性的一步：**「应用管理」→ 在页面里找「微信客服」。

> **入口在不在，就是整件事的答案。**
> 在 → 境外主体可以开，W1-W7 全部成立。
> 不在，或点进去提示不支持 → 这条路封死，方案作废。
> 按 CLAUDE.md 的高风险规则：**看一次，如实汇报，不要换主体连续重试。**

**第 2 步 —— 如果入口在，开通并打开 API：**

1. 打开「微信客服」旁边的开关
2. 打开「**通过API管理微信客服**」——不开这个，所有 API 调用都是白搭

**第 3 步 —— 建一个自建应用。**

⚠️ **这是起草方案时漏掉的一环。**官方「开发前必读」写得很清楚：微信客服的 API 是**由自建应用去调**的，微信客服本身不单独发 Secret。原文：「企业管理员可以在'企业微信管理后台-应用管理-微信客服应用'处，点开'API'小按钮，在'可调用接口的应用'中选择自建应用」。

1. 「应用管理」→「自建」→ 创建应用。它的 **AgentId + Secret** 就是调 API 用的凭据
2. 回到 微信客服 → 点开「API」小按钮 → 在「**可调用接口的应用**」里选上这个自建应用
3. 在「**通过API管理微信客服账号-企业内部开发**」里，勾上要交给 API 管的客服账号
4. **接待人员必须在这个自建应用的可见范围内**，否则接口返回 `errcode 60030`

**第 4 步 —— 回调配置**在自建应用的「接收消息服务器配置」里，三样东西：URL、`Token`、`EncodingAESKey`。URL 填 `https://chatbot.acuventech.com/webhook/wecom`——但**W4 做出来之前先别填**，企业微信存这个配置时会当场发一次回调验证，验不过存不下。
...
| `Secret` | **自建应用**的 Secret（不是微信客服的） |
| `Token` / `EncodingAESKey` | 自建应用 → 接收消息服务器配置 |
| `open_kfid` | 建好客服账号后用 API 拉列表拿，或后台看 |

对应到 `config.py`，新增四个必填环境变量：`wecom_corpid` / `wecom_secret` / `wecom_token` / `wecom_encoding_aes_key`。
**这个仓库是 PUBLIC，一律走环境变量、不给默认值**——照抄 `erp_password` / `crm_password` 现在的写法。
