...
- [ ] ⚠️ **企业可信IP：备案问题可能从另一扇门回来（2026-09-04 实测中发现）**
      ...
      **第 2 条要是成立，麻烦就大了**：多处资料称「配置企业可信IP前，请先设置可信域名或接收消息服务器URL」——而这两样**都要备案域名**。那样的话备案就绕不过去了，轮询也救不了，因为连 `sync_msg` 都调不动。
      ...
      ✅ **2026-09-04 已确认是原因 2**。在微信客服 →「Set apps that can call APIs」里勾选应用时，企业微信直接弹窗：

      > Apps "AI 客服机器人" **If a trusted IP is not configured, after clicking "Confirm", the app will not be set to have the capability to call APIs.**

      所以**可信IP 是硬性前置**，不配它这个授权勾了也不生效——刚才的 48002 就是这么来的。顺序是：**先配可信IP → 再回来授权**，反过来做无效。

      🚨 **2026-09-04 已确认：备案绕不过去。** 打开「Company's Trusted IP」弹窗，顶上一行写着：

      > Before configuring the company's trusted IP, please **Set trusted domain name** Or **Set the server URL for receiving messages**

      两条路都要域名，而「接收消息服务器URL」那条已经实测撞过 `Domain entity verification failed`。**这条依赖链是闭合的**：

      ```
      调微信客服 API  ←需要→  可信IP  ←需要→  可信域名 或 接收消息服务器URL  ←需要→  备案主体一致的域名
      ```

      **轮询这条逃生路也一并作废**——`sync_msg` 和其它接口一样被可信IP挡住，不是回调的问题。

      💡 **即使最坏情况，备案域名也只是用来「解锁可信IP这个设置项」**。可信IP 本身是个白名单，没有地域限制——真正调 API 的仍然是境外 VPS。所以那台大陆服务器只需要存在、让域名备案通过、应答一次 URL 验证，**不需要真的跑我们的服务**。这比原先设想的「大陆中转」轻得多。
...
## 六点五、2026-09-04 走到这里：一个需要你拍板的岔路

到今天为止，**能免费查的都查完了，代码能写的也写了**。现在卡在一个纯商业决定上，不是技术问题。

**已经确定成立的：**

- ✅ 主体（大陆个体工商户）、已认证、微信客服已开通、客服账号已建
- ✅ 自建应用已建，**Secret 正确**（`gettoken` 实测 errcode=0）
- ✅ W2（WXBizMsgCrypt）+ W4 的 GET 半边已完成，34 个测试，全套 288 passed
- ✅ `scripts/wecom_probe.py` 能一键复验整条链路

**已经确定不成立的：**

- ❌ 调任何微信客服接口 → 要可信IP → 要备案主体一致的域名。**闭合依赖，无绕过**
- ❌ 轮询代替回调这条逃生路，同样被可信IP挡住

### 岔路 A：走备案

| 项 | 代价 |
|---|---|
| 域名（阿里云/腾讯云，主体填个体户） | ¥30-70 / 年 |
| 大陆轻量服务器（**备案必需**，常要买满 3 个月） | ¥60-100 / 月 |
| ICP 备案审核 | **10-20 个工作日** |
| 合计 | **约 3-4 周 + ¥200-400** |

备案通过后：设可信域名或接收消息服务器URL → 解锁可信IP → 填 **VPS 的公网 IP** → 接口即通。

💡 那台大陆服务器**不需要跑我们的服务**，只需要存在、让域名备案通过、应答一次 URL 验证。真正调 API 的仍是境外 VPS（可信IP 白名单没有地域限制）。所以它是个空壳，不是中转。

⚠️ 残留风险：接入商会核查备案域名的解析去向，指向境外 IP 可能被注销接入——**可能在演示前夕失效**。让大陆那台实际承载 `/webhook/wecom` 可以规避，代价是多一跳跨境。

### 岔路 B：不做微信客服

已投入的东西**不会浪费**：`wecom_crypto.py`、`wecom_webhook.py`、探针脚本、这份方案，将来任何时候备案下来了，直接接着用。沉没成本约等于零。

### 建议

**倾向 B，除非有明确的中国客户在等这个渠道。** 理由是本文件第七节那个一直没被回答的问题：demo 的受众是马来西亚中小企业，他们的客户在 WhatsApp 上。花 3-4 周和一台大陆服务器换一个演示渠道，需要一个具体的人在等它，而不是「以后可能用得上」。

这是你的判断，不是我的——上面把价钱和时间都摆出来了，就是为了让这个判断好做。
