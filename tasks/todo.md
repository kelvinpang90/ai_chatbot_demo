...
- 行业档次：旗舰 `retail`（真 ERP / CRM）；深度 `food` + `realestate` + `hotel` + `saas`（自建轻量后端，记录进本系统 MySQL，统一后台 `/admin` 各一栏；`hotel` + `saas` 原是轻量档，任务 38.3 / 38.4 升上来）；`banking` 下架
...
"); curl -s -X POST "https://graph.facebook.com/v20.0/1066318999658447/publish" -H "Authorization: Bearer $TOKEN"'
  ...
  **同日跑了，答案是「门槛没解除」——verification 过了也没用**
  ...
  - ~~唯一可疑但没证实的：WABA 没有绑定付款方式~~——**同日当场试掉了，不是它**：用户绑了 Visa ···· 8409（Default），Overview 上那条 "Missing valid payment method" **警告随之消失**（证明 Meta 侧已登记，不是「刚绑还没生效」），再跑 curl **还是同一个 139000 / 4233020**
...
- [x] **任务 18：模板消息文案 + 提审**（用户任务，批次第一天就做）——**2026-09-22 完成：两个模板在 Meta 后台都是 Active（仅英文）**
  ...
  **✅ 2026-09-22：用户确认 `order_confirmed` / `order_shipped` 在 Meta 后台都是 Active，语言只提交了英文**（中文/马来文那两版没交，所以后台里不存在）。`notify.py:53` 的 `TEMPLATE_LANGUAGE = "en"` 和它上面那段注释因此是准确的，不用改。

  **⚠️ 状态达标 ≠ 这条路跑通了**。至今**一行真实模板流量都没发过**——payload 形状只有单测钉着，「Meta 认不认」要等真发一次才知道。这一步要往真人手机发消息，等用户点头（同任务 19 / 21 的规矩）。验的时候注意：模板只在 **24 小时窗口外**才走，窗口内一律纯文本，所以得挑一个超过 24 小时没跟 demo 号说过话的号码，或者临时绕过窗口判断。
  ...
  **写文案时定下来的几件事，别在后台改**
  ...
  - 模板固定发 **英文**。不是偷懒——模板是按语言分开审的，而客户档案里**没有语言字段可用**（`UserProfile.language` 至今没有任何地方写入，我查过全仓库）。窗口内的纯文本推送不受这个限制，那条是中/英/马三语一条
    **2026-09-22 更正**：`UserProfile.language` 现在有写入了（任务 38.1，`services/language.py:72`），所以上面那个理由已经不成立。**但结论不变**——后台只有英文那一版模板，中文/马来文的发出去就是 Meta 报错。想按客户语言发模板，得先回后台把另外两种语言各提交一次
...
## 批次 07：演示后的打磨（2026-09-17 起，用户边看边提）

- [x] **任务 38：WhatsApp 菜单中英双语 + 新客户发什么都先回菜单**——**2026-09-17 完成**（用户看菜单截图提的两条，方案 B + 「好」均由用户拍板）
  文件：`backend/app/routers/whatsapp_webhook.py`（`_send_bot_list`、`dispatch_message` 的不支持类型分支）、`backend/app/bots/registry.py`（新字段 `menu_title`）、五个 `bots/data/*.json`、`tests/test_whatsapp_webhook.py`

  **菜单双语**：按钮（也就是弹窗顶上那个标题）`Select` → `选择 Select`；顶部 `AI 客服演示 · AI Chatbot Demo`；正文中英两行；分组 `演示场景 Demo types`；**每行标题中英并列**。WhatsApp 行标题上限 24 字符，`Food Delivery Assistant 餐饮外卖点餐助手` 有 32 个，所以没有拿两个现有名字拼接，而是给每个 bot 加了一个**写好尺寸的 `menu_title`**（`餐饮外卖 Food Delivery` / `酒店旅游 Hotel & Travel` / `房产咨询 Real Estate` / `电商零售 Retail` / `技术支持 SaaS Support`，最长 19）。字段是必填的——新 bot 忘了写会直接加载失败，不会悄悄退回英文。描述行保持英文（用户选的方案 B 就是这样，中文描述加上去会超 72 字符上限）
  **新客户发什么都回菜单**：原来没选过 demo 的号码，发文字 / 语音 / 照片 / 文件本来就会自动收到菜单；**只有贴图、位置、名片这类会收到「本演示只能读文字…」**。现在这类消息如果来自还没选 demo 的人（陌生人，或刚发过 menu 的人），一样回菜单；已经在某个 demo 里的人照旧回那句提示。和「hi」一样**不建档**（用 `user_store.get`，不用 `get_or_create`），人工接管中也不插话
  **做不到、已跟用户说明**：WhatsApp 不允许替客户把弹窗打开，菜单只能以「带选择按钮的消息」送达，客户自己点才会弹出

  **验证**：新增 3 条测试先红后绿（菜单各处中英都在、每行标题等于 `menu_title` 且不超 24 且含中文、陌生人发贴图收到 list 且不留档）；原有「在对话里的人发贴图仍回提示」那条照旧绿。全套 **972 passed / 7 skipped**
  **没验**：真机上的排版（中英并列在手机弹窗里折不折行）——部署后要用户手机发一次 `menu` 看

- [x] **任务 38.1：固定话术按客户的语言回，不再三语一起发**——**2026-09-17 完成**（用户提出：截图里客户中文说「找人工客服」，回的是中英马三语；三点用户拍板：① 8 条固定话术 + 主动推送全改 ② 马来文用常见词判断可接受 ③ 不知道语言时保持三语）

  **一句话**：模型不写的那些话，现在按客户最近一次说话用的语言只发一种；从没读到过他写的字，才照旧三语。

  **怎么判断**（新模块 `backend/app/services/language.py`）：有汉字 → 中文（「我要买两个 fan, hantar ke Johor」也算中文，和 bot 回复规则一致）；命中一组常见马来词（saya / nak / boleh / tak / berapa / tolong…）→ 马来文；至少两个英文单词 → 英文；其余（`ok`、`hi`、表情、纯数字、`SKU-1001`）**判断不了，不改档案**——所以一个中文客户回一句 `ok` 不会被翻成英文。结果记在 `UserProfile.language`（这个字段以前存在但从没人写过）
  **只读客户自己的话**：文字、语音转写、照片/文件的**说明文字**；**不读文件名**（`Quotation_2026.pdf` 会被当成英文）、不读我们的 `[photo]` 标记、**不读点按钮**（按钮标题是我们写的字）。网页聊天也走同一条（`chat.py` 一行）

  **改了哪些话**：每条改成 `Localized(zh, en, ms)`，发送时 `.pick(语言)`，未知 → 用 ` / ` 连成三语
  - 转人工（`handover.HANDED_OVER_MESSAGE`）——语言在回复**之前**记下，所以「找人工客服」这一条本身就决定了回中文
  - 系统出错兜底（`llm.FALLBACK_REPLY`，按 `customer.language`）
  - 网关 5 条：每日限额、语音听不清、照片打不开、文件打不开、不支持的消息类型——这些路径**不建档**，所以按「这条消息里客户自己的字 → 档案上的语言 → 三语」的顺序挑
  - 房产表单被 Meta 拒收后的那句
  - 两条主动推送：零售下单确认（`erp.ORDER_PUSH`）、点餐已出餐（`food.ON_THE_WAY_PUSH`）——在工具调用时排队，那时客户档案就在手上（新增 `local.customer_language()`）
  - **范围外多改了一条，已跟用户说明**：导演台触发的「收尾总结」（`summary.compose`）原来是三段三语，现在按档案语言只发一段。它和上面同一个原则，当时盘点漏报了
  - **没改**：WhatsApp 模板仍只发英文（只提交了英文模板，`notify.py` 注释已更新）

  **验证**：
  - 新增 `tests/test_language.py`（判断规则、`ok` 不冲掉已知语言、三语拼接）+ `tests/test_replies_in_their_language.py`（截图场景重现、语言被记下、语音失败按档案语言、从未读到过仍三语、兜底马来文、表单被拒英文）+ 两条推送、收尾总结各一条——**接入各处的 8 条先红后绿**
  - 旧测试 19 条跟着改：不知道语言的客户比 `.pick(None)`（仍三语）；原来用英文句子「can I speak to a human please」「I want to see it」触发的两处，现在断言**只回英文**——这就是本任务要的行为变化。守卫「每条固定话术三语齐全」改成逐份检查中/英/马都在且马来文不等于英文（否则马来客户会收到空消息）
  - 全套 **997 passed / 7 skipped**

  **已知边角，没处理**：
  - 马来文词表之外的马来句子会被当成英文；只打一两个英文词的客户不会被切到英文
  - 推送的语言在**排队时**定：下单后 30 秒内客户突然换语言，推送仍是原来的语言
  - 档案里的 `language` 也会进给模型看的客户记录（`RECORD_FIELDS` 本来就包含它），无害，但它现在有值了

  **没验**：真机。部署后要用户用中文、英文、马来文各发一次「找人工客服 / talk to a human / nak cakap dengan manusia」看回复，外加一次点餐等推送

- [x] **任务 38.2：客户用自然语言说日期，bot 自己换算**——**2026-09-17 完成**（用户看真机截图提的：客人对酒店 bot 说「这个周末2天」，bot 回「我不知道今天是几号」，还要客人按 `YYYY-MM-DD` 打日期，举例用的是 2024 年）

  **根因**：**模型从来不知道今天是几号**。仓库里两处注释早就写明了（`llm.py` 的 `RECORD_FIELDS`、`realestate.py` 的看房日期校验），当时的补救是让酒店和房产的工具**在拒绝时**顺带说出今天的日期——可这次模型根本没调工具，直接反问客人，补救没生效。`2024-01-20` 是模型自己编的例子

  **修法**（`llm.py`）：每一轮给模型的「会变的那块」提示词末尾都加上 ① 今天（星期几 + 日期，马来西亚时间）② **接下来 14 天逐日列出星期几** ③ 规则 `DATES_IN_WORDS`：客人说「这个周末 / 下周三 / 明天 / next Friday / esok / hujung minggu ini」时自己从日历换算，用白话复述给客人确认；**不许让客人按格式打日期**，不许举日历之外的例子日期；只问话里真没说清的（哪个周末、住几晚）
  - 列 14 天而不只给今天：把「这个周末」换成日期是星期推算，正是模型容易算错的地方，给它日历只需要查
  - **不进缓存前缀**：日期每天变，放进去缓存每到午夜就作废
  - 时区：`date.today()` 靠 compose 里的 `TZ=Asia/Kuala_Lumpur`（和 `local._now()` 同一个惯例）。**实测**同一个 `python:3.13-slim` 基础镜像带 `/usr/share/zoneinfo`，设了 TZ 后 `tzname` 是 `+08`，所以凌晨 0-8 点不会算成前一天

  **验证**：
  - 新增 4 条测试先红后绿：提示词里有「Today is Wednesday 16 September 2026 (2026-09-16)」、14 天日历（周三那天「这个周末」对应的周六周日都在，第 15 天不在）、**两天的缓存前缀逐字相同**、规则在场。改了一条旧测试（匿名访客那块从「逐字相等」改为「以它开头」，因为末尾多了日历）。全套 **1001 passed / 7 skipped**
  - **真模型 + 真 dispatch 跑了 4 段**（酒店 bot，服务器日期 2026-09-17 周四）：截图原话「我想订槟城的房间，两个人」→「这个周末2天」→ **「9月19日（周六）入住，9月21日（周一）退房，共2晚，对吗？」并直接列出槟城房型**；「下周三入住住两晚」→ 9/23-9/25 ✓；「next Friday for 2 nights」→ Friday 25 – Sunday 27 并请客人确认（从周四说 next Friday 本就有歧义，问是对的）；马来文「hujung minggu ini, 2 malam」→ Sabtu 19 – Isnin 21 ✓。四段都没有要格式、没有错误年份
...
- [x] **任务 38.4：SaaS 工单写进数据库 + 后台接口**——**2026-09-17 完成**

  **一句话**：SaaS 客服 bot 开的工单现在写进共享 MySQL 的 `saas_tickets` 表，`GET /api/verticals/saas/tickets` 能列出所有客户的工单；客人档案里不再存工单。

  **做了什么**
  - 新增 `backend/app/verticals/saas/`（`models.py` 表 + `open_ticket` / `tickets` / `tickets_for`，编号 `TCK-00001` 由行 id 推出；`routes.py` 挂 token、库挂了 503）；`main.py` 挂路由
  - `tools/local.py`：`saas_create_ticket` / `saas_get_tickets` 改为读写这张表，按「渠道确认的客户 + 编号」查；库挂了回 `TICKET_NOT_OPENED` / `TICKETS_UNREADABLE`；没有客人档案回 `NO_CUSTOMER`。主题必填、优先级校验、长度截断这些原有规则不变；`saas_search_known_issues` 没动
  - **清掉了随之变成死代码的东西**：`_stored` / `_store` / `MAX_RECORDS` / 随机编号 `_reference` / `_now`。客人档案里给每个 bot 开的那格（`serving` 的 `store`、`records()`）**只剩点餐购物车在用**，保留；`local.py` 顶部说明、`_Serving` 注释、`food.py` 顶部那句一起改准
  - 原来测「写入落在路由手上的 profile」「坏掉的槽位会重建」的两条测试改用购物车测（这套机制还在用）；「记录不能无限增长」那条随 `MAX_RECORDS` 一起删了

  **偏离计划：防重复的规则和酒店不一样**。计划写的是「同主题同描述不开第二张」，但真模型实测时**每次措辞都不同**：客人说「帮我开个工单」bot 开了 `TCK-00002`；客人说「好的，开吧」bot 把主题从「报表导出卡在99%无法完成」改成「报表导出卡在99%」又开了 `TCK-00003`，还跟客人说「之前给您的编号有误」。字面比对完全拦不住。改成：**同一客户 15 分钟内已开过工单，默认不开新的，把那张返回给模型并附 `RECENT_TICKET` 说明**（同一问题就给这个编号、别说成新的或更正的）；**确实是另一个问题时，模型带 `different_problem=true` 再调一次才会开**。是不是同一个问题交给读得懂对话的模型判断，代码只负责不让它无意中重复开。酒店保持字段精确比对，因为预订是结构化字段、模型不会「改写」日期

  **验证**
  - 后端全套 **1023 passed / 7 skipped**。新增 `tests/test_verticals_saas.py`（按新到旧列出含客户和优先级、库挂 503、无 token / token 放 query 都 401、编号解析）；`test_local_tools.py` 新增编号顺序且不带客户信息、库挂了不假装开单、别人的编号查不到、无档案不能开、**换了措辞的重复报告仍只一张**、`different_problem=true` 能开第二张、一小时前的旧工单不挡新的；端到端那条（模型调工具 → 导演台事件）改为断言工单进了表
  - **变异检查 3/3 抓到**：拆掉 15 分钟防重复、查单不带客户条件、库挂了照样给编号
  - **真 MySQL 8**：建表、中文主题和描述 utf8mb4、重复报告返回原工单、别人查不到——全对
  - **真模型 + 真 MySQL 两轮对话**（就是撞出重复的那段）：第一轮开出 `TCK-00002`；第二轮「好的，开吧」模型又换措辞调了一次开单，**被挡回并拿到 `TCK-00002`**，表里这位客户只有一张，bot 回「工单编号确认为 TCK-00002」，没再说编号有误

  **已知边角**：工单状态是 `open`（待处理），真模型那次把它翻成了「处理中」——数据没错，措辞不精确；15 分钟内客户真的报了两个不同问题、而模型没意识到时，第二个问题会被挡回（模型看得到提示，可以再调一次）
  **没验**：线上真表（部署后首次使用自动建）；没有页面，等 38.5

- [x] **任务 38.5：统一后台页面——所有 demo 场景一个页面、每个场景一个栏目**——**2026-09-17 完成**
  - 新页面（暂定 `/admin`），栏目：**点餐 / 房产看房 / 酒店预订 / SaaS 工单**，**不放零售**（2026-09-17 用户拍板：零售的数据在真 ERP / CRM 里，统一页面只放我们自己的四个场景）；同一把 token；当前栏目记在地址里（例如 `/admin#hotel`），能直接收藏某个栏目
  ...
  **一句话**：`https://chatbot.acuventech.com/admin` 一个页面四个栏目，地址带 `#food` / `#realestate` / `#hotel` / `#saas`；旧的 `/food-admin`、`/vertical-admin` 打开后自动变成 `/admin#food`、`/admin#realestate`。

  **做了什么**
  - `FoodAdmin.tsx` 删掉，`VerticalAdmin.tsx` / `.css` 改名 `Admin.tsx` / `Admin.css`。两个页面原来各抄一份的「轮询 + 新行高亮 + 401 回 token 框 + 503 提示」收成一个 `useBackOffice` hook，四个栏目共用；表格内容照搬原来两页，酒店和工单是新写的
  - 酒店栏：编号、客人、房型 / 地点、入住→退房（几晚几位）、总价 / 每晚价、状态；**改过期的预订多显示一行「改过 09-17 14:03」**（改期不换编号也不算新行，不标一下看不出来）。工单栏：编号、客户、主题 + 描述、优先级（紧急红 / 高黄）、状态 `open` 显示「待处理」
  - 栏目是 `<a href="#hotel">` 链接，浏览器后退、手改地址都能切；**切栏目会卸载旧栏目**，所以只有当前栏目在轮询，切回来不会把其间进来的行误标成新的
  - `api.ts`：加 `readHotelBookings` / `readSupportTickets` 和对应类型；**`storedToken()` 清掉地址里的 `?token=` 时保留 `#`**——原来连 hash 一起抹掉，`/admin?token=…#hotel` 会落到默认栏目
  - `main.tsx` 路由加 `/admin`，两个旧地址指向对应栏目；`App.tsx` 的 `TokenGate` 改从 `Admin` 引（`title` / `blurb` 改成必填，原来的默认值是房产的，已经没人用）
  - 手机宽度：宽表格在面板里横向滚，不把整页撑宽；栏目按钮换行；编号列不折行

  **验证**
  - `npm run build`（含 `tsc -b`）通过，`oxlint` **0 warnings 0 errors**
  - **本地真栈进浏览器看**：MySQL 8 容器 + 后端容器（每张表先写种子数据）+ vite dev
    - `/food-admin?token=…#hotel` → 地址变成 `/admin#hotel`、打开酒店栏、token 被收走；酒店栏显示 `BK-00001` 带「改过」
    - 逐个点四个栏目：地址 hash 跟着变，点餐（`FD-00001` 制作中、时间线）、房产（预约 + 8 套房源）、工单（`TCK-00001` 高优先级、待处理）内容都对
    - **新行高亮**：停在酒店栏时往库里再写一批，`BK-00003` 被标高亮，**在工单栏期间进来的 `BK-00002` 切回来没被标**
    - **库挂了**：停掉 MySQL，接口回 503，工单栏头部和面板都显示「后台暂不可用：工单数据库连不上…」，没有显示「还没有工单」
    - **token 错**：塞一个错 token 打开 `/vertical-admin` → 地址变 `/admin#realestate`、回到 token 框带引号提示、错 token 被清掉；在框里填对的进去落在房产栏
    - **手机宽度 390px**（iframe 里量，窗口缩放在这台机器上不生效）：四个栏目都没有整页横向滚动，表格在面板内滚。第一版栏目按钮单行横滚、「SaaS 工单」被截一半，编号折成「TCK-」「00003」两行——改成换行 + 编号列不折行后复查通过
  - 后端没改，没跑后端测试

  **没验**：线上页面（部署后看）；真手机上摸（iframe 量的是布局，不是触摸）；本地容器没设时区所以时间显示是 UTC，线上容器是马来西亚时间，页面只切字符串不换算


- [x] **任务 38.6：文档跟上**（小改）——**2026-09-17 完成**
  ...
  **做了什么**：四项都照计划改了。**比计划多补了三处**：
  - **公开页原来就漏了餐饮订单和看房预约**——「存多久」「删除会覆盖」里只有对话记录和 ERP / CRM，自建后端那张表一直没写上。这次四类记录一起补，不只补酒店和工单
  - **「谁能看到」加了一句**（PDPA 中英文 + 公开页中英文）：同一把 console token 能打开 `/admin`，看到所有订单、预约、预订、工单连同姓名电话。统一后台把这些放在一个页面上，不写就少说了一类能看到资料的途径
  - 公开页和 PDPA 文档的「更新」日期改成 2026-09-17
  - 真机清单：开始前的标签改成 `/admin#realestate`；第 24 步改看「房产看房」栏；电脑上那一行（网页聊天订酒店、开工单）加上「后台酒店栏 / 工单栏出现同一个编号、再说一句『好的，开吧』工单仍只有一张」（38.4 撞出来的重复）；「会留下什么」改成演示后台
  - **没改** `docs/whatsapp-flows.md` 里的三处 `/vertical-admin`：旧地址会跳到 `/admin#realestate`，还能用

  **验证**：前端 build + oxlint 0；构建产物里搜到了中英文新句子。文档没有测试，逐句对过中英文意思一致。**没验**：线上 /privacy 页面部署后的样子（下面推上去后 curl 查）

- [x] **任务 38.7：统一后台 `/admin` 中英双语**——**2026-09-17 完成**（用户临时插入，排在 39.3 前；逐条追问后拍板：① 页面上一个中 / 英切换按钮、记住选择 ② 第一次打开默认英文，跟导演台一致）
  文件：`frontend/src/pages/Admin.tsx`、`frontend/src/pages/Admin.css`

  **做了什么**
  - `Admin.tsx` 里 ~90 处写死的中文全部进一本 `TEXT = { en, zh }` 字典：栏目名、表头、状态标签（已接单…已送达 / 紧急…低 / 待处理 / 已确认）、空列表提示、读取中 / 已更新 / 刷新、后台不可用两种提示、「改过」、晚 / 位、token 框标题说明按钮和 token 错误提示。**没并进 `i18n/strings.ts`**：那本是客户聊天页的三语字典，和这一页没有一个词重合
  - 切换按钮 `EN | 中文`：在栏目那一行的最右边，**token 框右上角也有一个**（进之前就能切）。选择存 `localStorage` 的 `admin_lang`，读写都包 try/catch，存不了就回到英文；同时改 `<html lang>`
  - **不翻的**（方案里的默认）：记录本身——客户名、菜名、地址、房型、房源类型和状态（Available / Under offer）、工单主题描述、客户填的看房时段——原样显示；品牌名；金额、日期、时间格式两种语言一样
  - **顺手改了一处结构，是切换能即时生效的前提**：原来「后台暂不可用：xx数据库连不上」这句是轮询出错时**拼好中文字符串存进 state** 的，切语言它不会变。改成 state 里只存失败种类（`'store' | 'list'`），显示时再按当前语言出句子；token 错误提示同理，从存句子改成存一个 `refused` 标志
  - 为了少写 5 份一样的 `<thead>`，抽了个 `HeaderRow`；`TABS` 从 `{id,label}` 改成只剩 id（标签进字典）；`STAGES` / `PRIORITIES` 拆成只管样式的 `STAGE_TAGS` / `PRIORITY_TAGS`
  - `TokenGate` 加了两个**可选**参数 `enter`（按钮字，默认「进入」）和 `aside`（放切换按钮）。**聊天页 `App.tsx` 一行没改**，不传就和原来一模一样

  **验证**
  - `npm run build`（含 `tsc -b`）通过，`oxlint` 0；脚本扫了字典以外的代码，剩下的中文只有切换按钮上的「中文」和 `TokenGate` 给聊天页的默认「进入」，都是故意的
  - **本地真栈进浏览器**：MySQL 8 + Redis + 后端容器（用 39.2 的 seed 灌了 36 张单据，另手工改出一张「制作中」的点餐单和一张「改过」的酒店预订）+ vite dev：
    - 清空 localStorage 打开 `/admin#food` → token 框是英文、`<html lang="en">`
    - 故意输错 token → 英文「Wrong token…」；在 token 框上点「中文」→ 标题、说明、按钮、**那条错误提示一起变中文**
    - 输对进去：点餐栏全中文（含「制作中」「含配送」「接单 · 出餐 · 送达」）；**刷新页面仍是中文**，`admin_lang=zh`
    - 点 EN 后脚本逐个切四个栏目，抓栏目 / 标题栏 / 表头 / 标签 / 空提示 / 表格里的灰字：全是英文——`Delivered / Preparing`、`Confirmed`、`Changed 09-11 19:54`、`2 nights · 4 guests`、`RM 480 / night`、`Normal / High / Urgent / Open`、`In … · Ready … · Delivered …`。扫出来的中文只有房产栏的「晚上7点」「下午3点」，是**客户填的看房时段（数据）**，按方案不翻
    - **390px 宽**（iframe 里量）：英文酒店栏、中文 SaaS 栏都没有整页横向滚动（scrollWidth = clientWidth = 373），栏目换行，切换按钮在右边完整显示，表格照旧在面板里横滚
    - 聊天页 `/` 的 token 框：「💬 Web chat」+ 中文说明 + 「进入」，**没有切换按钮**，和改之前一样

  **没验**：线上（部署后看）；真手机上摸；后台挂掉时那两句不可用提示没实际触发过（字典里两种语言都有，走的是同一个 `failureText`）

- [x] **任务 38.8：给客户看的产品小册子（中英两份 PDF）**——**2026-09-18 完成**（用户临时提出「功能整理」，逐条追问后拍板：给马来西亚中小企业老板看的通用产品介绍；中英各一份、6–10 页；只写现在能演示的五个行业；价格写起步价；截图用真实对话；中途加了「公司介绍」一页）
  文件：`docs/brochure/`（`brochure.html` 一份源文件、`build.py`、`screenshot-script.md`、`assets/`、`shots/`、导出的 `acuven-whatsapp-ai-en.pdf` / `-zh.pdf`）

  **一句话**：10 页 A4——封面（扫码试 demo）→ 痛点和方案 → 通用能力 → 零售 / 餐饮 / 酒店 / 地产 / SaaS 各一页（WhatsApp 对话截图 + 功能 + 后台截图 + 「试一试」原话）→ 关于 Acuven → 价格和联系方式。改完文案跑 `python docs/brochure/build.py` 重新出两份。

  **定下来的内容**
  - 价格：一次性开发费 RM 3,999 起、每年续费 RM 699 起、AI 用量按实际付费（用户给的数）
  - 联系：Acuven Technology / WhatsApp 01136182335 / admin@acuventech.com / acuventech.com；两个二维码 `wa.me/60173948123?text=menu`（demo）、`wa.me/601136182335`（联系）
  - 品牌：照官网 `acuven_os`（文字 Logo、`#1e6bff`、Inter）；公司介绍页的文案和四个数字（30+ 项目 / 15+ 年 / 20+ 客户 / 100% 按时）**照抄官网，没核实**
  - **只写线上真能用的**：e-Invoice 只说「开发票、PDF 发到 WhatsApp」（ERP 是 `MYINVOIS_MODE=mock`）；不提看房表单（Meta #139000）；不提要模板的隔天推送；不放导演台；银行已下线不写

  **做法**：一份 HTML，英文在标签里、中文在 `data-zh`（官网同款），`?lang=zh` 切换；无头 Chrome 打印成 PDF。用户截的是电脑版 WhatsApp（横图），`build.py` 按白 / 绿气泡的范围自动裁掉两边空墙纸再排版；后台图是在用户已登录的 Chrome 里截的（`/admin` 页放大 1.6 倍截；ERP 顶栏放大会挤坏，改为不放大只截表格那块）

  **偏离计划**
  - 行业页原本按竖版手机截图设计，用户给的是电脑版横图，改成「对话整宽在上、功能和后台并排在下」
  - 地产例子原写「蒲种三房，60 万以内」——demo 里没有这类房源，bot 两次都答「没有」、转推 Cheras。改成「Cheras 三房，50 万以内」（PROP-206）
  - 后台图原计划由用户截，最后改由我截；ERP / CRM 需要登录，由用户在浏览器里登好

  **验证**：两份 PDF 各 10 页 A4，逐页渲染成图看过；每页最低内容离页脚线至少约 4mm；三个二维码用 OpenCV 解码核对网址；后台图在打印分辨率下放大看过，最新一行（`SO-2026-00003`、`FD-00021`、`BK-00020`、看房线索、`TCK-00019`）看得清。顺带**第一次在线上确认了酒店、SaaS 两个后台栏目能用**（38.5 当时只在本地验过）

  **截图时撞见的产品问题（没修，待用户决定要不要开任务）**
  - ⚠️ **零售英文流程重复下单**：客户说 `yes` bot 下了 `SO-2026-00002`，客户补地址后又下了 `SO-2026-00003`，发票挂在后者。ERP 里 00002 一直停在 Confirmed。小册子英文零售截图里看得到两个单号（用户知情，保留）
  - 餐饮中文对话里，主动推送「Your order FD-00020 has left the kitchen…」是**英文**——38.1 说推送按客户语言发，这条没跟上
  - CRM 首页「New Leads Today」显示 0，但 Pipeline 的 Lead 栏最上面就是当天两张看房线索
  - 地产中文那段：bot 先问名字再介绍房源，最后只回「好的，已登记。稍后见！」，没复述看房时间和房源

  **没验**：打印出来的纸本效果（只看了屏幕渲染）；截图里的个人资料（姓名、住址、餐饮后台一行手机号）用户说零售截图保留原样，餐饮后台那行手机号已告知

- [x] **任务 38.9：公司介绍拆成单独一份 PDF + 小册子改两档价格**——**2026-09-19 完成**（用户提出「把公司简介分出来一个新的 pdf 版」，逐条追问后拍板：扩充成 3–6 页完整介绍、给中小企业老板看、和小册子同一套样式中英各一份、案例只写三个演示系统不写客户评价、每项服务写起步价；小册子删掉「关于」页）
  文件：`docs/brochure/company.html`（新）、`brochure.html`、`brochure.css` / `lang.js`（新，从 `brochure.html` 抽出的公用样式和语言切换）、`build.py`；导出 `acuven-company-profile-en.pdf` / `-zh.pdf`

  **一句话**：公司介绍 6 页——封面（四个数字）→ 关于我们（做事原则、服务过的行业）→ 7 项服务 → 三套可体验的演示（WhatsApp AI / CRM / ERP，截图 + 入口）→ 合作流程五步（3–18 周）→ 起步价表 + 联系方式。产品小册子删掉「关于 Acuven」变回 9 页，价格页改成两档。

  **用户给的数**（2026-09-19）
  - 公司数字：**100+ 交付项目、15+ 年经验、50+ 满意客户、100% 按时**（官网还是 30+ / 20+，**没改官网**，另议）
  - 起步价（开发费 / 年费）：WhatsApp AI 客服 RM 3,999 / 699；**AI 客服 + ERP + CRM 整套 RM 6,999 / 1,699**；CRM、ERP、POS 各 RM 3,999 / 1,299；AI 自动化 RM 5,999 / 1,299；数据看板 RM 2,999 / 999；官网 RM 699 / 399。WhatsApp AI 两档和 AI 自动化另加 AI 按用量
  - **38.8 小册子原来写「RM 3,999 含 AI 客服、ERP、CRM 和后台」是错的**：用户澄清 RM 3,999 只含 AI 客服 + 后台，含 ERP / CRM 的是 RM 6,999 整套。小册子价格页已改成两档并列，并注明零售、地产页里接 ERP / CRM 的功能属于整套

  **做法**：样式和语言切换抽成 `brochure.css` / `lang.js` 两份共用（中文标题改从 `<html data-zh-title>` 读）；**抽完先重导小册子逐页比像素，差异 0**，再动内容。`build.py` 一次出四份。演示页截图复用 38.8 的酒店对话、CRM 看板、ERP 销售单（浏览器扩展这次没连上，没截新的）。顺手删了两份都不再用的 `.prices` / `.builds` 样式

  **验证**：四份 PDF 逐页渲染看过（公司介绍 6 页、小册子 9 页），每页最低内容都在页脚线以上；两份公司介绍里的二维码用 OpenCV 解码核对。文案取自官网，没有另编客户评价和案例

  **没验**：纸本打印；公司介绍第 2 页下方留白较多（没有更多可用的真实素材，没硬填）
...
- [x] **任务 39.2：verticals 四张表的单据写入，日期回填**（food 订单 / 看房预约 / 酒店预订 / 工单，对话里引用的编号与表里一致）——**2026-09-17 完成**
  文件：`backend/app/tasks/seed_documents.py`（新增）、`seed_console.py`、`seed_lines.py`、`tests/test_seed_console.py`

  **做了什么**
  - food / hotel / saas / realestate 每个「行业 × 语言」的 6 位里，**3 位最近那段对话以一张单据收尾**：共 36 张（每行业 9 张）。retail 不动（39.3）
    - 点餐：说菜 → **真的** `food_update_cart`（输出就是真工具的）→ 报购物车和总价 → 给地址 → `food_place_order` → 回订单号 + 总价 + 10 分钟
    - 酒店：问房 → 真 `hotel_search_rooms` → 列房型 → 选一间 → `hotel_create_booking` → 回预订号 + 日期 + 总价
    - SaaS：报一个**不在已知问题里**的毛病 → 真 `saas_search_known_issues`（查不到）→ `saas_create_ticket` → 回工单号 + 优先级（一轮里两个工具）
    - 房产：报房源 + 日期 + 时段 + 名字 → `book_property_viewing` → 回「已提交，经纪人会打电话确认」
  - **单据经各自的 model 函数写**（`place_order` / `book` / `open_ticket` / `book_viewing`，就是真工具调的那几个），`at=` 传工具调用那一刻 → 总价、厨房时间线、`/admin` 的行都和真单一样生成。**控制台上的工具输出**：酒店 / 工单用 model 自带的 `for_the_guest()` / `for_the_customer()`，看房用 `realestate._confirmation()`，都和真工具一字不差；**点餐下单那条是照着工具的 dict 逐字段重建的**（真工具会用 `time.time()` 盖时间戳，没法回填），用了 `food._priced/_menu/_fee/_rm` 这几个私有函数保证价钱同源
  - **酒店入住日一律在今天之后**：`local._stay` 会拒绝过去的入住日，seed 就是拿它算的房价 / 晚数 / 总价；副作用是 `/admin` 酒店栏看到的是「即将入住」的预订，这正好
  - **顺序**：清单据 → 生成对话（边生成边写单据，拿到编号填进回复）→ 审计库事务。两个库之间没有事务：审计那步挂了，会留下没有对话的单据，**下次跑先清掉**，自愈
  - **偏离一处：看房表没有 `customer_key` 列**（只有姓名 + 电话），按前缀清不了。没改产品表结构、也没往电话栏里塞标记（`/admin` 会显示出来），而是**在 verticals 库里建了一张 seed 自己的登记表 `seed_console_viewings`**，记下写过的看房 id，按它删。seed 的看房电话留空（和隐藏号码的客户一样）
  - `main` 现在要求 **`VERTICALS_MYSQL_URL` 也配着**，输出多一项 `N documents`

  **验证**
  - 容器里全套 **1066 passed / 10 skipped**（本文件 29 → 46 条）
  - 新测试钉住：36 张 / 每行业 9 张；回复里的编号就是写进去的那张；**单据时间 = 建单工具那一行的时间**；点餐总价 = 菜单价 × 数量 + 运费；酒店每一笔 `local._stay` 重算结果不变且入住日 ≥ 今天；4 个工单问题**确实查不到**已知问题；不给 filer 就不写单据；清理只删三张表的 `ZZ.SEED%` + 登记表里的看房
  - **变异检查**：酒店回复填错编号、点餐单据晚 60 秒、清理漏掉工单表——各自对应的那条测试变红
  - **真 MySQL 8（两个库，照 `deploy/mysql-init`）+ Redis 7**，先往四张表各写一张真单（`60173948123`）：
    - 连跑两次，四张表都是「真 1 + seed 9」、看房 10 行、登记表 9 行，**没有翻倍**
    - 27 条带 FD- / BK- / TCK- 的回复，逐个去表里查：**编号存在且挂在同一位客户名下，0 处不符**；9 次看房调用的 id 和登记表完全一致
    - 抽一张酒店预订：工具调用 `2026-08-25 11:54:18.743`，表里 `booked_at` 同一毫秒
    - `/api/verticals/{food/orders,hotel/bookings,saas/tickets,realestate/viewings}` 全 200、各 10 行，最上面是今天的真单；seed 的点餐状态是 `delivered`
    - 读回一段中文点餐对话：说菜 → 购物车卡 → 报价 RM 69.90 → 地址 → `FD-00011` 下单卡 → 「订单号 FD-00011，共 RM 69.90」→ 道谢
    - `--clear` 之后四张表只剩真单、登记表 0 行
  - 实跑里看到中文点单写成「我要2 份」，模板改成「2份」（纯文案，改后没重跑实跑，测试不依赖这个字）
...
- [x] **任务 39.5：Claude 生成 15 段精品 JSON**（用户审）——**2026-09-22 完成**
  文件：`backend/app/tasks/data/showcase.json`（新增，对话本体）、`backend/app/tasks/seed_showcase.py`（新增，加载 + 校验）、`seed_console.py`、`tests/test_seed_console.py`

  **做了什么**

  - **15 段，五个行业 × 三种语言各一段**，每段 3-5 轮（模板那些是 1-2 轮）。占的是每个「行业 × 语言」六个槽位里的**最后一个（i=5）**——前面五个都已经有主了（39.2 的单据、39.3 的 ERP 采购员、39.4 的线索），这是唯一空着的。**日期落在过去 7 天内**而不是 28 天：列表按时间倒序，这 15 段是给人点开的，得在靠上的位置
  - **写的是客户的话和 bot 的回复，不写工具输出**。工具输出照旧由 seed 当场调真工具拿（39.1 立的规矩）。每一轮再写一个 `expects`，列出这条回复从输出里引了哪些字符串；**seed 发现工具不再返回这些字符串就整批 raise**——「酒店改了房价、回复里还念着旧价」这种错会在导演台上挂一整天，和 39.3「ERP 连不上就 raise」同一条理由
  - **回复里的每个 RM 数字必须有出处**：bot 自己的 `context_data`、某个工具的输出、或者客户自己刚说过的数。bot 合理自己算出来的那几个（10% 首付、月供、四个人的早餐）逐轮写进 `derived`——**这样「没对过账的数字」是一份看得见的清单**，不用翻着找。一条参数化测试把这条规则钉死
  - **能调的工具是一份白名单，只有三个**：`food_update_cart` / `hotel_search_rooms` / `saas_search_known_issues`。理由有两条，一条硬一条软：硬的是**这些对话每晚都要跑一遍**，会写东西的工具就会每晚留下一条没人聊过的记录，而单据是 39.2 的、线索是 39.4 的，它们各自负责清理；软的是这三个工具的答案来自本仓库的 `app/bots/data/*.json`，所以手写的回复**对得上、而且对得起测试**（ERP 的答案在别人家的库里，对不了）
  - **`request_human_help` 是唯一一个「只画卡、不调用」的**：真调会给客户打上接管标记、还会往实时流里发一条 span，批次 07 明写这两样都不 seed。卡片内容用的是这个工具自己的返回常量 `human.HANDED_OVER`，所以导演台上看到的就是真卡；它本该设的那个标记没设。15 段里有 2 段这么收尾（retail/en 查不到别人号码下的单、food/zh 客人对贝类过敏要改菜），而且**这种轮只能是最后一轮**——工具的返回原文就是「转完就闭嘴」，后面再接一句就是 bot 盖着同事说话
  - **精品段不接模板那句道谢收尾**：`_conversation` 多了一个 `closing` 参数。自己写的结尾被一句「好的，谢谢！」盖掉就白写了，何况那 2 段刚转完人工
  - 15 段各写一行 `note` 说明这段是干什么的，给审的人看，**不进导演台**

  **15 段各是什么**（每段都在「这个 bot 真的能这么答」的范围内）

  - retail zh 月结 + 运费 + COD + 退货的 B2B 询价，最后没买 / **retail en 同事用别的号码下的单，bot 说明自己只看得到本号码、拒绝按公司名查，转人工** / retail ms 东马运费、COD 老实拒绝、退款时效、客服时间
  - food zh 犹豫 → 建购物车 → 中途换菜重新算 → **过敏要改菜，转厨房** / food en 20 人公司订餐，RM 300 / 24 小时规则 + 真算出 RM 354.60 / food ms 点菜单上没有的 nasi kandar，bot 说没有并给最接近的
  - hotel zh 一家四口，搜出来正好两间住得下的 / **hotel en 一段里搜两次**（兰卡威限价 + 槟城）/ hotel ms 蜜月，兰卡威全表 + 早餐 + 接机
  - **realestate zh 被问「上周约的看房几点」，bot 直说自己看不到预约系统**（persona 原文就是这么要求的）+ 房贷粗算 / realestate en 外国人买房，**bot 拿 RM 1,000,000 门槛把自家的 PROP-203 排除掉** / realestate ms 问到 `Under offer` 的 PROP-207，照实说有人先出价了
  - saas zh Slack 令牌 90 天过期 / saas en 仪表盘慢 → 归档项目 → 真的变快了 → 席位 / saas ms 登录失败其实是客户自己的 SSO，让他找 workspace admin

  **偏离计划的一处**

  - 计划只说「~15 段精品由 Claude 一次生成」，没说占哪个槽、能不能调工具。**retail 的三段一个 ERP 卡都没有**——因为 ERP 的答案没法对着本仓库校验，手写回复会对不上。retail 的 ERP 戏在 39.3 那 9 段采购员对话里已经有了，这三段补的是模板给不了的东西：讨价、老实拒绝、转人工。这是我自己拍的板，觉得不对的话说一声

  **验证到什么程度**

  - 单测：容器里全套 **1139 passed / 10 skipped**（基线 1074→1090，本次净增 49）。`test_seed_console.py` 70 → 119 条
  - **变异检查 7 处，7 处全红**：精品段接回模板道谢 / 槽位改到 i=4（撞上线索）/ `SHOWCASE_DAYS` 改成 28 天 / `request_human_help` 真的调 / 去掉 `expects` 的校验 / 把一个算出来的数字从 `derived` 里拿掉冒充查来的 / 把一个会写的工具放进白名单——每一处都有对应的测试变红
    - ⚠️ 头一版里**槽位和日期那两处变异没红**，因为测试读的正是它要检查的那个常量，常量一改测试跟着改。改成不读常量（槽位比对「文件里的第一句话」找出来再算位置，日期用写死的 7 天）之后才红
  - 新测试钉住的：15 段覆盖 5×3 且一格一段；每个工具都在该 bot 自己的 tools 里、且在白名单内；**逐段拿真工具跑一遍，`expects` 的每个字符串都还在**（15 条参数化）；**逐段检查回复里每个 RM 数字都有出处**（15 条参数化）；转人工的卡是工具原文、且全程 `handover.begin` 一次都没被调、90 个 profile 没有一个带接管标记；精品段的最后一句是自己写的那句、不是模板道谢，且客户的每一句和文件逐句对得上；文件本身的 11 条拒绝规则（不认识的 bot / 语言 / 少于三轮 / 用了该 bot 没有的工具 / 用了会写的工具 / 有工具没 `expects` / 没工具却写了 `expects` / `derived` 的数字不在回复里 / 多余字段 / 转人工不在最后一轮 / 同一格写了两段）
  - **本地容器里把 15 段按导演台的排版整个打印出来逐段读过**：开场白是 bot 自己的 disclaimer，工具卡的内容是真输出，回复里的每个价钱和卡片里的对得上（如 food/zh 两次改单 RM 40.80 → RM 40.40，food/en RM 354.60），日期落在 09-16 到 09-21（今天 09-22）

  **线上真跑一次**（2026-09-22 部署后当场手动补跑，命令由用户执行；`docker exec` 被 Claude 这边的权限分类器拦）

  - `Cleared: 1140 rows, 90 profiles, 15 CRM rows` → `Seeded: 90 customers, **731 messages, 129 tool calls**, 437 model calls, 36 documents, 15 leads, 9 ERP buyers`，exit 0
  - **39.2 / 39.3 / 39.4 一项没被碰坏**：36 张单据、15 张线索、9 位 ERP 采购员，和 39.4 那次逐项一致。CRM 正好清掉 15 行（标记写错的话这里会是 0），15 次 DELETE 全 200、15 次 POST 全 201，之后没有任何 `DELETE /api/deals`
  - **日志里 `request_human_help` 一个 HTTP 请求都没发**——这正是设计：那张卡是工具自己的返回常量，没真调用、没打接管标记
  - 上一批（03:30 的 cron，39.4 的代码）是 1140 行，这一批 731+129+437 = **1297 行**，多出来的就是 15 段精品换掉 15 段模板

  **15 段在导演台上是哪 15 位**（key_id 和名字都是确定的，不随机）：`ZZ.SEED0006` 王美玲 / `0012` Marcus Tan / `0018` Mohd Azlan / `0024` 杨欣怡 / `0030` Grace Loh / `0036` Amirul Hakim / `0042` 叶素芬 / `0048` Hannah Low / `0054` Irfan Kamal / `0060` Vivian Chong / `0066` Meera Das / `0072` Aiman Shah / `0078` Crystal Khoo / `0084` Samantha Gan / `0090` Arif Ramli——每格六位里的最后一位

  **没验的**

  - **浏览器里没看导演台**：这 15 位点开长什么样、工具卡怎么渲染，没有肉眼看过（前端一行没改）
  - 今晚 03:30 的 cron 会是第一次**无人值守**带精品段跑
  - 15 段的文字本身**等用户审**——这是任务里写明的那一步，我只保证「bot 真的能这么答」和「数字对得上」，读起来像不像人话是用户说了算
  - **没做「先校验文件再清库」**：`seed_showcase.load()` 在 `plan()` 里，也就是在清单据和清 CRM 之后。ERP 那条规矩（39.3）是因为 ERP 是会宕的外部系统，而这个 JSON 和代码在同一个镜像里、测试挂了就发不出去，为一个发生不了的故障加前置校验是投机代码。真觉得该加，一行的事
