...
- **本地开发/验证用 Docker**：本地机器上已有 Docker 环境（跑着其他项目用的 Redis、MySQL，与本项目无关，本项目不使用它们，维持内存存储设计）。为了让本地验证的运行环境和 VPS 部署环境尽量一致，后端从项目早期就用 `docker compose` 跑起来做验收（而非裸 `uvicorn`/venv），源码用 volume 挂载支持热重载，兼顾开发效率。前端在开发阶段仍用 `npm run dev`（Vite 热更新体验更好，且前端产物是静态文件，运行时环境差异不像后端那么敏感），到整体联调阶段再补前端 Dockerfile 并入同一个 `docker-compose.yml`，部署到 VPS 时用的是本地已经验证过的同一份 compose 配置。
